#Ứng dụng SmL_Mp3_V8 game đa ngôn ngữ. images Game đoán chữ có gửi ảnh kèm điểm
#==========Text to mp3 đa ngôn ngữ V1.0 _tích hợp A.i, Discor, Amazon ASW ,Gtts....
import time
import warnings
import os
import re
...
# ⬇️ Tự động copy file Excel nếu chưa có trong AppData
...
def pick_existing_asset(*names):
    for name in names:
        ...
        if os.path.exists(candidate):
            return candidate
    return os.path.join(BASE_FOLDER, names[0])
...
FONT_LATIN = os.path.join(BASE_FOLDER, "DejaVuSans.ttf")
FONT_CJK = os.path.join(BASE_FOLDER, "SamsungGothicKorean.ttf")
...
##    LOGO_PATH = os.path.join(BASE_FOLDER, "logo.png")
ICON_ICO_PATH = os.path.join(BASE_FOLDER, "logo.ico")
        ...
        def thread_xuat():
            ...
            try:
                for i, (dong, lang) in enumerate(danh_sach_doc):
                    ...
                    try:
                        # Nếu rỗng sau làm sạch → chèn im lặng 300ms
                        if not dong_sach.strip():
                            ...
                            # Nạp đoạn âm thanh; nếu lỗi → im lặng 300ms
                            try:
                        ...
                        msg = str(e)
                        if "No text to speak" in msg or "No text to send to TTS API" in msg:
                            print("⚠ Dòng rỗng/không hợp lệ → chèn im lặng 300ms")
                            full_audio += AudioSegment.silent(duration=300)
                        else:
                            raise
                    finally:
                    ...
                    label_status.config(text=f"Đang xuất dòng {i+1}/{tong_dong}")
            ...
            try:
                for file_idx in range(tong_file):
                    ...
                    for dong, lang in group:
                        ...
                        dong_da_xu_ly += 1
                        progress["value"] = int(dong_da_xu_ly / tong_dong * 100)
                        label_status.config(text=f"Đang xuất file {file_idx + 1}/{tong_file} - dòng {dong_da_xu_ly}/{tong_dong}")
                        popup_progress.update_idletasks()
...
def create_frame_noi_dung(parent):
    ...
    # Khung đọc đề & xuất mp3 sát mép phải (cả chiều dọc)
    frame_doc = tk.LabelFrame(frame_noi_dung, text="🎧 Đọc Nội Dung & Xuất MP3 VCJ School", font=("Arial", 8, "bold"), bg="#f8fff8")
    frame_doc.place(x=1030, y=8, width=320, height=660)

    btn_doc = tk.Button(frame_doc, text=" Đọc/Xuất Mp3-Video", font=("Arial", 14, "bold"),
    ...
    btn_xuat_mp3 = tk.Button(frame_doc, text="🎧 Xuất file MP3", font=("Arial", 8), command=xuat_file_mp3, bg="lightblue")
...
root.title("Text To MP3 ĐA NGÔN NGỮ V8-2025 Game Đoán Chữ- Cty TNHH DL&TM Quốc Tế Việt Trung Nhật 0986183806 - TT GIÁO DỤC QUỐC TẾ VCJ International School")
...
# Tải ảnh gốc bằng PIL
pil_img = Image.open(LOGO_PATH)

# Resize (ví dụ vừa khít frame_logo)
pil_img = pil_img.resize((150, 150), Image.LANCZOS)  # hoặc kích thước mong muốn
